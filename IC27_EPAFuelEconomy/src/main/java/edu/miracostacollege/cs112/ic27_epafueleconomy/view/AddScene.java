...
public class AddScene extends Scene {
    ...
    private void save() {
        ...
        //DONE: If any of the error labels are visible, return.
        if (yearErrorLabel.isVisible() || makeErrorLabel.isVisible() || modelErrorLabel.isVisible() ||
                cityMPGErrorLabel.isVisible() || highwayMPGErrorLabel.isVisible() ||
                annualFuelCostErrorLabel.isVisible() || fuelEconomyRatingErrorLabel.isVisible())
            return;
        //DONE: Otherwise, instantiate a new Vehicle object and add it to the list of all vehicles.
        controller.addVehicle(new Vehicle(year, make, model, cityMPG, highwayMPG, annualFuelCost, fuelEconomyRating));
        //DONE: Then, go back to the previous scene.
        goBackToPrevScene();
    }
