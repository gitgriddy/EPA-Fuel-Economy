...
public class MainScene extends Scene {
    ...
    public MainScene() {
        ...
        //DONE: Add a listener on the selected item property of the vehiclesLV
        //DONE: In the changed(...) method, assign the selected vehicle to the new value
        //DONE: If the selected vehicle is null (no vehicle selected), then disable the removeButton
        //DONE: Otherwise, it should be enabled.
        ...
        // DONE: For the slider, set its mouse dragged event to call filter()
        // Wire up the slider to call filter() just like the combobox
    ...
    private void removeVehicle() {
        //DONE: If the selected vehicle is null, just return (nothing to do)
        if (selectedVehicle == null)
            return;
        //DONE: Otherwise, remove the selected vehicle from the back-end (MODEL) first, then the front-end (VIEW)
        controller.removeVehicle(selectedVehicle);  // back-end, from the Model (binary file)
        vehiclesList.remove(selectedVehicle);  // visually, from the View
